...
export const validateForm = (data) => {
  ...
  if (/^R\$\s0,00/g.test(data.contribution_revenue)) {
    errors.push(['contribution_revenue', 'Valor não pode ser 0.']);
  }

  if (data.deadline_revenue.length < 1) {
    errors.push(['deadline_revenue', 'Campo obrigatório.']);
  } else {
    if (/^(0|\D)/.test(data.deadline_revenue)) {
      errors.push(['deadline_revenue', 'Deve ser número maior que 0.']);
    }
  }
  ...
  if (/^R\$\s0,00/g.test(data.contribution_index)) {
    errors.push(['contribution_index', 'Valor não pode ser 0.']);
  }

  if (data.rentability_index.length < 1) {
    errors.push(['rentability_index', 'Campo Obrigatório']);
  } else {
    if (/^%/g.test(data.rentability_index)) {
      errors
          .push([
            'rentability_index',
            'O valor precisa ser numérico acima de 0.',
          ]);
    }
  }
